...
import Material.Table.Sortable as Sortable
...
init people =
    let
        model =
            { people = people
            , tableState = Sortable.initialSort "Year" Table.Descending
            , mdl = Material.model
            }
    in
        ( model, Cmd.none )
...
view : Model -> Html Msg
view model =
    Sortable.view (config model) model.tableState model.people
        |> Material.Scheme.top


config : Model -> Sortable.Config Person Msg
config model =
    Sortable.config
        { toId = .name
        , toMsg = SetTableState
        , columns =
            ...
            , Sortable.intColumn "Year" .year
            , Sortable.stringColumn "City" .city
            , Sortable.stringColumn "State" .state
            ]
        }
